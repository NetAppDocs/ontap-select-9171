...
summary: Find answers to common questions for ONTAP Select.
---
= FAQ for ONTAP Select
:hardbreaks:
:nofooter:
:icons: font
:linkattrs:
...
* If the ONTAP Select VM was also upgraded to a Large instance (using the Premium XL license), then reverting to a prior version before 9.6 is not supported, because the Large instance feature is not available in earlier versions.
...
Changes to the following ONTAP Select cluster properties are recognized by ONTAP Select Deploy using the cluster refresh operation available through the UI, CLI, or REST API:
...
Modifying the networking details of the Deploy instance after it's running in an environment is not supported. For more information, see the Knowledge Base article link:https://kb.netapp.com/onprem/ontap/ONTAP_Select/Modifying_DNS_configuration_of_ONTAP_Deploy_instance[Modifying DNS configuration of ONTAP Deploy instance^].
...
All other errors are handled by ONTAP HA in the case of a two-node or four-node cluster. If the hypervisor server needs to be replaced and the ONTAP Select cluster needs to be reconstituted with a new server, refer to _Can I recover an ONTAP Select node?_.

.Can I recover an ONTAP Select node? 

Contact NetApp Technical Support if you need to recover an ONTAP Select node, for example, if an ONTAP Select VM or host is completely lost or a single-node cluster is accidentally deleted. For more information, see the Knowledge Base article link:https://kb.netapp.com/on-prem/ontap/ontap-select/Select-KBs/ONTAP_Select_node_VM_recovery_in_case_of_disaster[ONTAP Select node VM recovery in case of disaster^].
...
ONTAP Select Deploy uses the VMware VIX API to communicate with the vCenter, ESX host, or both. The VMware documentation states that the initial connection to either a vCenter Server or an ESX host is done using HTTPS/SOAP on TCP port 443. This is the port for secure HTTP over TLS/SSL. Secondly, a connection to the ESX host is opened on a socket on TCP port 902. Data going over this connection is encrypted with SSL. Additionally, ONTAP Select Deploy issues a `PING` command to verify that there is an ESX host responding at the IP address you specified.
...
MetroCluster SDS is a lower-cost synchronous replication option that falls under the category of the MetroCluster Business Continuity solutions from NetApp. It is available only with ONTAP Select, unlike NetApp MetroCluster that is available on FAS Hybrid Flash, AFF, and NetApp Private Storage for Cloud.

.How is the MetroCluster SDS different from NetApp MetroCluster?

MetroCluster SDS provides a synchronous replication solution and falls under NetApp MetroCluster solutions. However, a key difference is the connectivity type (only IP networks are supported rather than FC and IP).
...
The two-node MetroCluster SDS is defined as a cluster with nodes that are physically separated (different rooms, different buildings, or different data centers) and each node’s uplink connections are connected to separate network switches. Although MetroCluster SDS does not require dedicated hardware, the environment should support the minimum requirements for latency (5ms RTT and 5ms jitter for a max total of 10ms).
...
* 5ms RTT and 5ms jitter for a max total of 10ms 
...
// 2026 MAR 23, ONTAPDOC-3806
// 2025 SEP 8, ONTAPDOC-3320
// 2025 JULY 15, ontap-select-internal-issue-206
