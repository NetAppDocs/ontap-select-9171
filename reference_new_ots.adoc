...
Enhanced support for cluster expansion and contraction on ESXi hosts::
Beginning with ONTAP Select 9.17.1, cluster expansion and contraction is supported for between six-node and twelve-node clusters on ESXi hosts:
+
--
You can increase the cluster size for an existing ESXi cluster in the following increments:

* From six nodes to eight, ten, or twelve nodes
* From eight nodes to ten or twelve nodes
* From ten to twelve nodes 

You can decrease the cluster size for an existing ESXi cluster in the following increments:
...
Support for SnapMirror cloud::
Beginning with ONTAP Select 9.17.1, SnapMirror cloud is supported with ONTAP Select. SnapMirror cloud is not licensed by default for ONTAP Select. When you deploy an ONTAP Select 9.17.1 cluster or upgrade an existing ONTAP Select cluster to version 9.17.1, you need to request a SnapMirror cloud evaluation license. For more information, refer to link:reference_lic_ontap_features.html#other-ontap-features[ONTAP features that are separately licensed].


Support for SnapLock Select::
Beginning with ONTAP Select 9.17.1, new deployments have SnapLock Select (including tamperproof snapshot locking) licensed automatically. For upgrades from ONTAP Select 9.16.1 and earlier, you can download the SnapLock Select license free of charge from the link:https://mysupport.netapp.com/site/[NetApp Support Site^] and apply it manually. For more information, refer to link:reference_lic_ontap_features.html#enabled-ontap-features[ONTAP features automatically enabled by default].
...
Beginning with ONTAP Select 9.17.1, KVM hypervisor is supported on Red Hat Enterprise Linux (RHEL) 10.1, 10.0, 9.7, and 9.6 and Rocky Linux 10.0, 9.7, and 9.6.
+
include::./_include/rhel-and-rockylinux10-rl-support.adoc[]

Enhanced VMware ESXi support::
ONTAP Select 9.17.1 includes support for VMware ESXi 9.0.
