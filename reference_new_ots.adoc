...
Beginning with ONTAP Select 9.17.1, new deployments have SnapLock Select (including tamperproof snapshot locking) licensed automatically. For upgrades from ONTAP Select 9.16.1 and earlier, you can download the SnapLock Select license free of charge from the link:https://mysupport.netapp.com/site/[NetApp Support Site^] and apply it manually. For more information, see link:reference_lic_ontap_features.html#enabled-ontap-features[ONTAP features automatically enabled by default].
...
Beginning with ONTAP Select 9.17.1, KVM hypervisor is supported on Red Hat Enterprise Linux (RHEL) 10.1, 10.0, 9.7, and 9.6 and Rocky Linux 10.1, 10.0, 9.7, and 9.6.
+
