...
Support for SnapLock Select Enterprise::
Beginning with ONTAP Select 9.17.1, new deployments have SnapLock Select Enterprise (including tamperproof snapshot locking) licensed automatically. For upgrades from ONTAP Select 9.16.1 and earlier, you need to download the SnapLock Select license from the link:https://mysupport.netapp.com/site/[NetApp Support Site^] and apply it manually.
...
Beginning with ONTAP Select 9.17.1, KVM hypervisor is supported on Red Hat Enterprise Linux (RHEL) 10.0 and 9.6 and Rocky Linux 10.0 and 9.6.
+
include::./_include/rehl-and-rockylinux10-rl-support.adoc[]
