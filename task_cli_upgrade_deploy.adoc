...
[.lead]
Upgrade an existing ONTAP Select Deploy utility virtual machine in-place using the ONTAP Select Deploy utility CLI.

.Before you begin

Verify that ONTAP Select Deploy is not used to perform any other tasks during the upgrade. See the link:reference_release_notes.html[release notes] for information and restrictions about upgrading the ONTAP Select Deploy utility.
...
You can upgrade directly to ONTAP Select Deploy 9.18.1 from ONTAP Select Deploy 9.17.1 or 9.16.1. To upgrade from ONTAP Select Deploy 9.15.1 or earlier, review the release notes for your ONTAP Select version.
====

== Step 1: Download the upgrade package

To begin the upgrade process, download the ONTAP Select Deploy Upgrade file from the NetApp Support Site. The upgrade package is formatted as a single compressed file.
...
. Select and download the appropriate *ONTAP Select Deploy Upgrade* package. Respond to all prompts as needed.
...
. In a command shell on your local workstation, use the `scp` (Secure Copy Protocol) utility to upload the image file to the ONTAP Select Deploy virtual machine, as shown in the following example:
+
....
